...
class Dino{
  ...
  call(){
    return `${this.name} says ${this.roar}`;
  }
}
...
const uni = graduates.filter(uniName => uniName.university.includes('Uni') )
...
*/
 ...
 const lowerCase = zooAnimals.map ( littleName => littleName.animal_name.toLowerCase() );
...
const lowerPopulation = zooAnimals.filter(dyingOut => dyingOut.population < 5)
...
*/
const populationTotal = zooAnimals.reduce((totalPopulation, inputPopulation) => totalPopulation + inputPopulation.population, 0  );
